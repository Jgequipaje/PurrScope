...
describe("filterUrls — Property 8: Filter excludes by sourceSitemap", () => {
  it(
    "excludes all entries whose sourceSitemap is in excludePatterns (http URLs), includes all others",
    () => {
...
describe("filterUrls — Property 9: Combined filter applies patterns before scope", () => {
  it(
    "result equals first removing excludePattern entries then applying scope filter",
    () => {
        ...
        fc.property(
          ...
          (entries, scope, excludePatterns) => {
            ...
            const afterScope = afterExclude.filter(({ sourceSitemap }) => {
              ...
              if (scope === "static")
                return sourceSitemap.endsWith("sitemap-static.xml");
              if (scope === "dynamic")
                return sourceSitemap.endsWith("-dpages.xml");
              return false;
            });
