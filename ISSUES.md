...
This is misleading — 10 URLs is the maximum allowed, so the scan should proceed normally without any warning. The red warning should only appear when the user attempts to go _over_ the limit (i.e. 11+).
...
Note: TC-05 in the same file tests the _current_ (buggy) behavior with 11 URLs. The new `ISSUE-01` test verifies the _fixed_ behavior — exactly 10 URLs should not trigger the warning.
