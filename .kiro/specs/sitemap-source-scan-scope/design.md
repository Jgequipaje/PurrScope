...
const SCOPE_OPTIONS = [
  { value: "all", label: "All Pages", description: "Scan every URL discovered in the sitemap." },
  {
    ...
    description: "Only URLs from sitemap-static.xml.",
  },
  {
    value: "dynamic",
    label: "Dynamic Pages",
    description: "Only URLs from *-dpages.xml sitemaps.",
  },
];
```
...
_For any_ urlset sitemap fetched during a crawl, every URL extracted from that sitemap should have a `sourceSitemap` value equal to the filename portion of the sitemap URL (e.g. `sitemap-static.xml`), not the full URL path.
...
_For any_ crawl over a sitemap tree where the same URL appears in multiple child sitemaps, the final `pageUrls` list should contain that URL exactly once, and its `sourceSitemap` should be the filename of the first sitemap in which it was encountered (depth-first order).
...
_For any_ completed crawl, every URL in `pageUrls` should have a corresponding non-empty entry in `sourceSitemapMap`. That is, `sourceSitemapMap[url]` should be a non-empty string for all `url` in `pageUrls`.
...
_For any_ completed crawl, the sum of all values in `perSourceCounts` should equal `pageCount`, and for each sitemap filename `f`, `perSourceCounts[f]` should equal the number of entries in `sourceSitemapMap` whose value is `f`.
...
_For any_ URL list and source map, filtering with scope `"all"` (and no exclude patterns) should return every URL in the input list unchanged.
...
_For any_ URL list and source map, filtering with scope `"static"` should return only URLs whose `sourceSitemapMap` entry is exactly `"sitemap-static.xml"`. No URL with a different source should appear in the output.
...
_For any_ URL list and source map, filtering with scope `"dynamic"` should return only URLs whose `sourceSitemapMap` entry ends with `"-dpages.xml"`. No URL with a non-matching source should appear in the output.
...
_For any_ URL list, scope, and source map, every URL in the filter output should be a URL that was present in the input list. The filter should never construct or invent URLs.
...
_For any_ array of `ScanResult` objects, the rendered `ResultsTable` should contain a "Source Sitemap" cell for each row showing the `sourceSitemap` value when present, or an em dash (`—`) when `sourceSitemap` is absent.
...
_For any_ `SitemapCrawlResult` with a non-empty `perSourceCounts`, the rendered `SitemapDebug` component should display each sitemap filename and its corresponding URL count.
...
_For any_ `SitemapCrawlResult` and any two different `ScanScope` values, rendering `SitemapDebug` with each scope should display the `totalAfterFiltering` value from the corresponding `FilterResult` without triggering a new crawl.
...
_For any_ URL included in a scan, the `ScanResult` for that URL should have a `sourceSitemap` field equal to `sourceSitemapMap[url]` from the originating `SitemapCrawlResult`.
...
_For any_ title string, `titleStatus` should be `"Pass"` if and only if the title length is in the range [45, 61] inclusive, and `"Fail"` otherwise.
...
_For any_ meta description string, `descriptionStatus` should be `"Pass"` if and only if the description length is in the range [145, 161] inclusive, and `"Fail"` otherwise.
